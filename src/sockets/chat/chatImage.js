    ...
    socket.on('chat-image', (data) => {
      if (data.groupId) {   
        ...
        if (clients[data.groupId]) {
          clients = emitNotifyToArray(clients,data.groupId, io, "response-chat-image", response);
        }
      }
      if (data.contactId) {
        let response = {
        ...
        if (clients[data.contactId]) {
          clients = emitNotifyToArray(clients,data.contactId, io, "response-chat-image",response);
        }
      }
    });
