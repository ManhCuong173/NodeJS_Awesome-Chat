    ...
    socket.on('user-is-typing', (data) => {
      if (data.groupId) {   
        ...
        if (clients[data.groupId]) {
          clients = emitNotifyToArray(clients,data.groupId, io, "response-user-is-typing", response);
        }
      }
      if (data.contactId) {
        let response = {
        ...
        if (clients[data.contactId]) {
          clients = emitNotifyToArray(clients,data.contactId, io, "response-user-is-typing",response);
        }
      }
    });
