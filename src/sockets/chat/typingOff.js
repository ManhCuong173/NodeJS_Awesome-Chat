...
let typingOff = (io) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('user-is-stop-typing', (data) => {
      if (data.groupId) {   
        let response = {
        ...
        if (clients[data.groupId]) {
          clients = emitNotifyToArray(clients,data.groupId, io, "response-user-is-stop-typing", response);
        }
      }
      if (data.contactId) {
        let response = {
        ...
        if (clients[data.contactId]) {
          clients = emitNotifyToArray(clients,data.contactId, io, "response-user-is-stop-typing",response);
        }
      }
    });
